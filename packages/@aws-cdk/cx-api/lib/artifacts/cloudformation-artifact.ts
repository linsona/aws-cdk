...
export abstract class CloudFormationArtifact extends CloudArtifact {
  /**
   * The file name of the template.
   */
  public abstract readonly templateFile: string;
   ...
   */
  public abstract readonly stackTemplateAssetObjectUrl?: string;
   ...
   */
  public abstract readonly environment: Environment;
   ...
   */
  public abstract readonly assumeRoleArn?: string;
   ...
   * @default - false
   */
  public abstract readonly validateOnSynth?: boolean;

  /**
   * CloudFormation tags to pass to the stack.
   */
  public abstract readonly tags: { [id: string]: string };
   ...
   */
  public abstract readonly assumeRoleExternalId?: string;
}

export class CloudFormationStackArtifact extends CloudFormationArtifact {
  /**
   * The file name of the template.
   */
...
export class CloudFormationStackSetArtifact extends CloudFormationArtifact {
  /**
   * The file name of the template.
   */
  public readonly templateFile: string;
  ...
  constructor(assembly: CloudAssembly, artifactId: string, artifact: cxschema.ArtifactManifest) {
    ...
    const properties = (this.manifest.properties || {}) as cxschema.AwsCloudFormationStackSetProperties;
    if (!properties.templateFile) {
      throw new Error('Invalid CloudFormation stack artifact. Missing "templateFile" property in cloud assembly manifest');
    }
    if (!artifact.environment) {
    ...
    // from the stack metadata
    this.tags = properties.tags ?? this.tagsFromMetadata();
    this.regionConcurrencyType = properties.regionConcurrencyType;
    this.stackTemplateAssetObjectUrl = properties.stackTemplateAssetObjectUrl;

    this.stackSetName = properties.stackSetName || artifactId;
    this.assets = this.findMetadataByType(cxschema.ArtifactMetadataEntryType.ASSET).map(e => e.data as cxschema.AssetMetadataEntry);

    this.displayName = this.stackSetName === artifactId
      ? this.stackSetName
      : `${artifactId} (${this.stackSetName})`;

    this.name = this.stackSetName; // backwards compat
    this.originalName = this.stackSetName;
  }
